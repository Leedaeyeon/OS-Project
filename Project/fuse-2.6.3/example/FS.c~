...
static const char *backing_path = "/root/Desktop/fuse-2.6.3/example/parsedexample.xml";
//static char *file_path="/dir4";
//static char *file_str = "Testing testing 123 HI\n";
static char *file_path, *file_str;
...
void remove_node(TREE* tree, char* path){
 NODE *cur, *tmp, *node, *parent;
 ...
 if(node->firstChild != NULL){
  printf("ERROR: cannot remove! directory not empty!");
  return;
 }
 parent = get_parent(tree, path);
 if(node == parent->firstChild){
  ...
  while(cur->nextSib != node){
   cur = cur->nextSib;
  }
  tmp = cur->nextSib;
  cur->nextSib = cur->nextSib->nextSib;
  free(tmp);
 }
}
...
static int hello_mkdir(const char *path, struct stat *stbuf){
	...
	tmp_path = (char*)malloc(sizeof(char)*200);
...
static int hello_rmdir(const char *path){
	int res;
	    ...
	    char* rm_tmp_path;
	    rm_tmp_path = (char*)malloc(sizeof(char)*200);
...
static int hello_rmfile(const char *path){
	int res;
...
file_str = (char*)malloc(sizeof(char)*10000);
file_path = (char*)malloc(sizeof(char)*200);
tmp_curnode = (char*)malloc(sizeof(char)*200);
...
printf("path at hello_open: %s\n", path);
printf("file path at hello_open: %s\n", file_path);
    ...
    if(strcmp(path, backing_path)==0){
	//deparse(tree);
	printf("yo deparse yo\n");	
	}

    if((fi->flags & 3) != O_RDONLY){
	printf("am i here instead\n");
       return -EACCES;
    }

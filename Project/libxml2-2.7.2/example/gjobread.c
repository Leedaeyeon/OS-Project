...
printPerson(personPtr cur) {
	...
	if(cur->name) printf("/%s",cur->name);
	if(cur->email) printf("/%s\n", cur->email);

FILE *file = NULL;
file = fopen("/mnt/hgfs/Project/fuse-2.6.3/project/path.txt","w");

if(file==NULL){
	printf("Error writing path to file.\n");
	exit(1);
}

if(cur->name) fprintf(file, "/%s", cur->name);
if(cur->email) fprintf(file, "/%s\n", cur->email);
